...
class NN(object):
    def __init__(self, l0, l1, l2, lambd, batch_size=6):
        self.lr = 0.01                                  # 学习率
    ...
    # 反向传播
    def backward(self):
        dW2 = np.dot(self.a1.T, self.d2) / self.batch_size   #  用于反向传播
        db2 = np.sum(self.d2, axis=0) / self.batch_size
 
        d1 = np.dot(self.d2, self.W2.T) * Dsigmoid(self.z1)
...
if __name__ == '__main__':
    ...
    plt.plot(np.arange(0,1.0,0.1),precision_matrix)
    plt.show()
